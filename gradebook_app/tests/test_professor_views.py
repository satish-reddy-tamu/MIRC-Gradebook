...
class TestProfileViews(TestCase):
    ...
    def test_add_course_configuration(self):
        Course.objects.create(course_code = "313", name = "student", description = "description", department = "CSCE", year= 2023, semester = "Fall", credits = "3" )
        evaluations ={"eval1": {'name': "abc", 'eval_type': "type", 'weight': 0.5, 'max_marks': 50.0},"eval2": {'name': "abc2", 'eval_type': "type2", 'weight': 0.5, 'max_marks': 50.0}}
    ...
    def test_get_update_evaluation(self):
        evaluations ={"1": {'name': "abc", 'eval_type': "type", 'weight': 0.5, 'max_marks': 50.0},"eval2": {'name': "abc2", 'eval_type': "type2", 'weight': 0.5, 'max_marks': 50.0}}
        session = self.client.session
        session['evaluations'] = evaluations
        session['eval_id'] = "1"
        session.save()
    ...
    def test_post_update_evaluation_valid_form(self):
        ...
        session['eval_id'] = "1"
        session.save()
        response = self.client.post(reverse('update_evaluation', args=[1, 1]), obj, HTTP_X_REQUESTED_WITH ='XMLHttpRequest' )
        self.assertEquals(response.status_code, 200)
        self.assertTrue(response.json()['form_is_valid'])
    ...
    def test_post_add_grade_function_valid_form(self):
        ...
        response = self.client.post(reverse('add_grade_function', args=[1]), obj, HTTP_X_REQUESTED_WITH ='XMLHttpRequest' )
        self.assertEquals(response.status_code, 200)
        self.assertTrue(response.json()['form_is_valid'])